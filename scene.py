...
class GeneralizedIVT(Scene):
    def construct(self):
        # Title in LaTeX
        title = Tex(r"Intermediate Value Theorem for $\mathbb{R}^n$").to_edge(UP)
        self.play(Write(title))

        # A as a connected region in R^n (represented in 2D for visualization)
        A_text = Tex(r"Let $A \subseteq \mathbb{R}^n$ be a connected set").scale(0.5).next_to(title, DOWN)
        ...
        x_points_text = Tex(r"Take any two distinct $x_1,x_2 \in A$").scale(0.5).next_to(title, DOWN)
        ...
        f_A_text = Tex(r"$f(A)$ is connected and a subset of $\mathbb{R}$, hence an interval").scale(0.5).next_to(title, DOWN)
        ...
        f_x_text = Tex(r"Then $f(x_1), f(x_2) \in f(A)$ implies that $f(x_1)<f(x_2)$ without loss of generality").scale(0.5).next_to(title, DOWN)
        ...
        c_text = Tex(r"Which means that there exists $c \in f(A)$ such that $f(x_1)<c<f(x_2)$").scale(0.5).next_to(title, DOWN)
        ...
        xc_text = Tex(r"And since $c \in f(A)$, there exists $x_c \in A$ such that $f(x_c)=c$").scale(0.5).next_to(title, DOWN)
        ...
        theorem_text = Tex(
            r"Therefore, given $f: A \subseteq \mathbb{R}^n \to \mathbb{R}$ continuous,",
            r" and $A$ connected,",
            r" then for $x_1, x_2 \in A$, if $f(x_1) < f(x_2)$,",
            r" $\forall c \in (f(x_1), f(x_2))$,",
            r" there exists $x_c \in A$ such that $f(x_c) = c$."
